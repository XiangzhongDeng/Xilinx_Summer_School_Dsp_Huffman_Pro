...
void huffman(
    ...
    #pragma HLS INTERFACE axis register both port=symbol_histogram_value_V
	#pragma HLS INTERFACE axis register both port=symbol_histogram_frequency_V
    ...
    for(int i = 0; i < INPUT_SYMBOL_SIZE; i++) {
        ...
        symbol_histogram[i].value = symbol_histogram_value_V[i];

        symbol_histogram[i].frequency = symbol_histogram_frequency_V[i];
    }
